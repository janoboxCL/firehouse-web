...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const commerceOrder = `CONCURSO-${Date.now()}-${crypto.randomUUID().slice(0, 6)}`;
    ...
    try {
      ...
      const pago = await crearPagoFlow(
        { apiKey: context.env.FLOW_API_KEY!, secretKey: context.env.FLOW_SECRET_KEY!, baseUrl: context.env.FLOW_BASE_URL! },
        {
          ...
          urlReturn: `${siteUrl}/sorteo/gracias?orden=${commerceOrder}`,
          optional: { codigo: rifaCodigoTexto ?? '', tickets: numeros.join(',') },
        },
      );
