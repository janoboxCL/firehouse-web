...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const { data: config, error: errConfig } = await supabase
    ...
    .eq('id', 1)
    .single();
  if (errConfig || !config) return jsonResponse(500, { error: 'config_no_disponible' });
  ...
  if (errVenta || !venta) return jsonResponse(500, { error: 'no_se_pudo_crear_la_venta' });
  ...
  if (errReserva) return jsonResponse(500, { error: 'no_se_pudo_reservar' });
